...
### Rendering Engine
- Displays the requested content by parsing HTML and CSS.
...
- Then, it parses CSS, to associate style informations with each DOM node to construct the **CSSOM Tree**. 
...
- Then, execute a **layout process** where each node is positioned on the screen with coordinates.
  - Calculate the exact position and size of each node within the viewport.
